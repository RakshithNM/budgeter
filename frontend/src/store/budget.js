...
export const useBudgetStore = defineStore("budget", {
    ...
    getters: {
        totalBudget: (state) => state.budgets.reduce((sum, item) => sum + (item.effectiveAmount ?? item.amount), 0),
        ...
        totalSpent: (state) => {
            const [year, month] = state.month.split("-").map(Number);
            if (!year || !month)
                return state.spends.reduce((sum, item) => sum + item.amount, 0);
            const start = new Date(Date.UTC(year, month - 1, 1));
            const end = new Date(Date.UTC(year, month, 1));
    ...
    actions: {
        ...
        async fetchRollingCashflow() {
            ...
            const latest = report.trend[report.trend.length - 1];
        ...
        async setCategoryBudget(categoryId, amount) {
        ...
        async setCategoryTarget(categoryId, targetAmount) {
